...
def _consolidated_report(conn: sqlite3.Connection) -> dict:
    ...
    for row in balances:
        ...
        price_usd = latest_prices.get(symbol, 0.0)
        usd_source = "snapshot"
        snapshot_at = None
...
class PortfolioRequestHandler(BaseHTTPRequestHandler):
    ...
    def _handle_list_accounts(self, parsed: parse.ParseResult) -> None:
        ...
        try:
            with _db_connection() as conn:
                rows = core.search_accounts(conn, raw_query) if raw_query else core.list_accounts(conn)
    ...
    def _handle_account_detail(self, account_id_raw: str) -> None:
        ...
        try:
            with _db_connection() as conn:
                detail = core.get_account_detail(conn, account_id)
            if detail is None:
    ...
    def _handle_list_currencies(self) -> None:
        try:
            with _db_connection() as conn:
                rows = core.list_currencies(conn)
    ...
    def _handle_add_currency(self) -> None:
        try:
            ...
            with _db_connection() as conn:
                core.add_currency(conn, simbolo, nombre)
    ...
    def _handle_delete_currency(self, simbolo_raw: str) -> None:
        ...
        try:
            with _db_connection() as conn:
                core.delete_currency(conn, simbolo)
            _response(self, {"ok": True})
        except ValueError as exc:
            _error(self, str(exc))
        except sqlite3.Error as exc:
    ...
    def _handle_market_prices(self, parsed: parse.ParseResult) -> None:
        ...
        try:
            ...
            precio_usd = prices.fetch_market_prices(symbol)
            ...
            if precio_usd is None:
                try:
                    coin_id = prices.SYMBOL_TO_COINGECKO_ID.get(symbol)
                    if coin_id:
                        query_params = parse.urlencode({"ids": coin_id, "vs_currencies": "usd"})
                        url = f"{prices.COINGECKO_API_URL}?{query_params}"
                        data = prices.fetch_json(url)
    ...
    def _handle_list_recent_movements(self, parsed: parse.ParseResult) -> None:
        ...
        try:
            ...
            with _db_connection() as conn:
                rows = core.get_recent_movements_all(conn, limit=limit)
    ...
    def _handle_create_movement(self) -> None:
        try:
            ...
            fecha = fecha_raw if fecha_raw else core.now_iso()
            ...
            with _db_connection() as conn:
                ...
                movement_id = core.insert_movement(
                    conn,
                    account_id=account_id,
                    fecha=fecha,
                    monto=amount,
    ...
    def _handle_update_movement(self, movement_id_raw: str) -> None:
        try:
            ...
            with _db_connection() as conn:
                current = core.get_movement_by_id(conn, movement_id)
                if current is None:
    ...
    def _handle_delete_movement(self, movement_id_raw: str) -> None:
        try:
            movement_id = int(movement_id_raw)
            with _db_connection() as conn:
                deleted = core.delete_movement_by_id(conn, movement_id=movement_id)
            if not deleted:
                _error(self, "Movement not found", status=HTTPStatus.NOT_FOUND)
                return
    ...
    def _handle_staking_progress(self) -> None:
        try:
            with _db_connection() as conn:
                rows = core.get_staking_progress(conn)
    ...
    def _handle_update_prices(self) -> None:
        try:
            with _db_connection() as conn:
                prices.update_prices_from_internet(conn)
            _response(self, {"ok": True, "updated": True})
    ...
    def _handle_latest_prices(self) -> None:
        try:
            with _db_connection() as conn:
                items = _latest_prices(conn)
            _response(self, {"ok": True, "items": items})
        except sqlite3.Error as exc:
            _error(self, f"Database error: {exc}", status=HTTPStatus.INTERNAL_SERVER_ERROR)
    ...
    def _handle_delete_backup(self, filename_raw: str) -> None:
        filename = parse.unquote(filename_raw).strip()
        # Only allow deleting files inside the backups directory — never the active DB
        ...
        if os.path.normpath(target_path) == os.path.normpath(db.get_db_path()):
    ...
    def _handle_db_switch(self) -> None:
        ...
        # Resolve against the data directory or its backups subdirectory
        candidates = [
            os.path.join(db.DATA_DIR, filename),
            os.path.join(db.backups_dir(), filename),
        ]
        ...
        try:
            ...
            db.set_active_db_path(previous_path)
